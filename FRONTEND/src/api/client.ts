...
import { 
  Department, Employee, Category, EmissionFactor, EnvironmentalGoal,
  ...
  PointsTransaction, Redemption, EmployeeBadge, EsgConfig
} from '../types';
...
const defaultGoals: EnvironmentalGoal[] = [
...
const defaultPolicies: EsgPolicy[] = [
...
const defaultCsrActivities: CsrActivity[] = [
...
const defaultParticipations: EmployeeParticipation[] = [
...
const defaultChallengeParticipations: ChallengeParticipation[] = [
...
const defaultPolicyAcknowledgements: PolicyAcknowledgement[] = [
...
const defaultDepartmentScores: DepartmentScore[] = [
...
const defaultPointsTransactions: PointsTransaction[] = [
...
const defaultRedemptions: Redemption[] = [];
const defaultEmployeeBadges: EmployeeBadge[] = [
...
export const db = {
  get config(): EsgConfig { return getStorageItem('config', defaultConfig); },
  set config(val: EsgConfig) { setStorageItem('config', val); },

  get departments(): Department[] { return getStorageItem('departments', defaultDepartments); },
  set departments(val: Department[]) { setStorageItem('departments', val); },

  get employees(): Employee[] { return getStorageItem('employees', defaultEmployees); },
  set employees(val: Employee[]) { setStorageItem('employees', val); },

  get categories(): Category[] { return getStorageItem('categories', defaultCategories); },
  set categories(val: Category[]) { setStorageItem('categories', val); },

  get emissionFactors(): EmissionFactor[] { return getStorageItem('emissionFactors', defaultEmissionFactors); },
  set emissionFactors(val: EmissionFactor[]) { setStorageItem('emissionFactors', val); },

  get goals(): EnvironmentalGoal[] { return getStorageItem('goals', defaultGoals); },
  set goals(val: EnvironmentalGoal[]) { setStorageItem('goals', val); },

  get policies(): EsgPolicy[] { return getStorageItem('policies', defaultPolicies); },
  set policies(val: EsgPolicy[]) { setStorageItem('policies', val); },

  get badges(): Badge[] { return getStorageItem('badges', defaultBadges); },
  set badges(val: Badge[]) { setStorageItem('badges', val); },

  get rewards(): Reward[] { return getStorageItem('rewards', defaultRewards); },
  set rewards(val: Reward[]) { setStorageItem('rewards', val); },

  get carbonTransactions(): CarbonTransaction[] { return getStorageItem('carbonTransactions', defaultCarbonTransactions); },
  set carbonTransactions(val: CarbonTransaction[]) { setStorageItem('carbonTransactions', val); },

  get csrActivities(): CsrActivity[] { return getStorageItem('csrActivities', defaultCsrActivities); },
  set csrActivities(val: CsrActivity[]) { setStorageItem('csrActivities', val); },

  get participations(): EmployeeParticipation[] { return getStorageItem('participations', defaultParticipations); },
  set participations(val: EmployeeParticipation[]) { setStorageItem('participations', val); },

  get challenges(): Challenge[] { return getStorageItem('challenges', defaultChallenges); },
  set challenges(val: Challenge[]) { setStorageItem('challenges', val); },

  get challengeParticipations(): ChallengeParticipation[] { return getStorageItem('challengeParticipations', defaultChallengeParticipations); },
  set challengeParticipations(val: ChallengeParticipation[]) { setStorageItem('challengeParticipations', val); },

  get policyAcknowledgements(): PolicyAcknowledgement[] { return getStorageItem('policyAcknowledgements', defaultPolicyAcknowledgements); },
  set policyAcknowledgements(val: PolicyAcknowledgement[]) { setStorageItem('policyAcknowledgements', val); },

  get audits(): Audit[] { return getStorageItem('audits', defaultAudits); },
  set audits(val: Audit[]) { setStorageItem('audits', val); },

  get complianceIssues(): ComplianceIssue[] { return getStorageItem('complianceIssues', defaultComplianceIssues); },
  set complianceIssues(val: ComplianceIssue[]) { setStorageItem('complianceIssues', val); },

  get departmentScores(): DepartmentScore[] { return getStorageItem('departmentScores', defaultDepartmentScores); },
  set departmentScores(val: DepartmentScore[]) { setStorageItem('departmentScores', val); },

  get pointsTransactions(): PointsTransaction[] { return getStorageItem('pointsTransactions', defaultPointsTransactions); },
  set pointsTransactions(val: PointsTransaction[]) { setStorageItem('pointsTransactions', val); },

  get redemptions(): Redemption[] { return getStorageItem('redemptions', defaultRedemptions); },
  set redemptions(val: Redemption[]) { setStorageItem('redemptions', val); },

  get employeeBadges(): EmployeeBadge[] { return getStorageItem('employeeBadges', defaultEmployeeBadges); },
  set employeeBadges(val: EmployeeBadge[]) { setStorageItem('employeeBadges', val); }
};
...
export const getSessionUser = (): Employee => {
  return db.employees.find(e => e.id === currentUserId) || db.employees[0];
};

// Helper for gamification triggers (XP updates)
export const addXPAndPoints = (employeeId: string, amount: number, type: 'CSR' | 'CHALLENGE' | 'ADJUSTMENT', sourceId: string) => {
  // Update employee XP
...
const checkBadgeUnlocks = (employeeId: string) => {
...
export const mockHandlers = {
  ...
  createEmissionFactor: async (data: Omit<EmissionFactor, 'id'>) => {
    await latency();
  ...
  createCarbonTransaction: async (data: Omit<CarbonTransaction, 'id' | 'co2Amount' | 'date'>) => {
    await latency();
    const factors = db.emissionFactors;
    ...
    // Calculate emissions
    // If calculating fuel or other things, multiply by quantity (sourceId is parsed, or standard quantity input)
    // We'll treat the sourceId string or quantity input as multiplier
    // Let's assume a default multiplier of 500 if not specified, or parse it
    const quantity = 500; 
    ...
    if (db.config.autoEmissionCalc) {
      ...
      if (primaryGoalIdx !== -1) {
        // Adding transactions increases our tracked emissions (or progress in reducing, let's just increment tracked)
  ...
  createGoal: async (data: Omit<EnvironmentalGoal, 'id'>) => {
    await latency();
    const goals = db.goals;
  ...
  createCSRActivity: async (data: Omit<CsrActivity, 'id'>) => {
    await latency();
    const acts = db.csrActivities;
  ...
  createParticipation: async (activityId: string, proofUrl: string | null) => {
    await latency();
    ...
    const newPrt: EmployeeParticipation = {
      id: `prt-${Date.now()}`,
  ...
  approveParticipation: async (id: string) => {
    await latency();
    const prts = db.participations;
    ...
    // Trigger Points transaction and award XP
    const p = prts[idx];
  ...
  rejectParticipation: async (id: string) => {
    await latency();
    const prts = db.participations;
    const idx = prts.findIndex(p => p.id === id);
    if (idx === -1) throw new Error('Participation not found');
  ...
  createPolicy: async (data: Omit<EsgPolicy, 'id'>) => {
    await latency();
    const pols = db.policies;
  ...
  acknowledgePolicy: async (id: string) => {
    ...
    // Award minor XP for policy reading
  ...
  createAudit: async (data: Omit<Audit, 'id'>) => {
    await latency();
    const auds = db.audits;
  ...
  createComplianceIssue: async (data: Omit<ComplianceIssue, 'id' | 'createdAt'>) => {
    await latency();
    const issues = db.complianceIssues;
  ...
  updateComplianceIssueStatus: async (id: string, status: ComplianceStatus) => {
    await latency();
    const issues = db.complianceIssues;
  ...
  createChallenge: async (data: Omit<Challenge, 'id'>) => {
    await latency();
    const chs = db.challenges;
  ...
  participateInChallenge: async (challengeId: string) => {
    await latency();
    const currentUser = getSessionUser();
    const prts = db.challengeParticipations;
    ...
    const newPrt: ChallengeParticipation = {
      id: `chp-${Date.now()}`,
      challengeId,
      employeeId: currentUser.id,
      progress: 0,
  ...
  updateChallengeProgress: async (participationId: string, progress: number, proofUrl: string | null) => {
    await latency();
    const prts = db.challengeParticipations;
    ...
    // Auto-approve if no evidence required and progress is 100%
    if (progress === 100) {
      if (ch && !ch.evidenceRequired) {
        prts[idx].approvalStatus = 'APPROVED';
        prts[idx].xpAwarded = ch.xp;
        
        // Award XP to Employee immediately
  ...
  approveChallengeParticipation: async (id: string) => {
    await latency();
    const prts = db.challengeParticipations;
    const idx = prts.findIndex(p => p.id === id);
    if (idx === -1) throw new Error('Participation not found');
  ...
  rejectChallengeParticipation: async (id: string) => {
    await latency();
    const prts = db.challengeParticipations;
    const idx = prts.findIndex(p => p.id === id);
    if (idx === -1) throw new Error('Participation not found');
  ...
  getLeaderboard: async () => {
    ...
    // Sort employees by XP descending
  ...
  createReward: async (data: Omit<Reward, 'id'>) => {
    await latency();
  ...
  redeemReward: async (id: string) => {
    ...
    // Add negative transaction to points ledger
    const txs = db.pointsTransactions;
    txs.push({
      id: `ptx-${Date.now()}`,
      employeeId: currentUser.id,
  ...
  updateConfig: async (data: Partial<EsgConfig>) => {
    await latency();
  ...
  createDepartment: async (data: Omit<Department, 'id' | 'employeeCount' | 'createdAt' | 'updatedAt'>) => {
    await latency();
    const depts = db.departments;
